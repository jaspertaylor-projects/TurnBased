...
export function CardStudioSection({
  ...
  function updateRow(next: CardStudioRow) {
    update({ ...state, rows: state.rows.map((row) => (row.id === next.id ? next : row)) });
  }
  ...
  function generate() {
    try {
      ...
      if (!cards.length) throw new Error("Add a card with at least one copy first.");
      update({
        ...state,
        generated: {
          generatedAt: new Date().toISOString(),
          sourceFingerprint: fingerprint,
          cards: cards.map(({ id, sourceRowId, copyNumber }) => ({ id, sourceRowId, copyNumber })),
        },
      });
      setView("deck");
      setPage(0);
      setStatus(
        `Generated ${cards.length} cards from ${state.rows.length} designs. Your deck is ready to print and playtest.`,
      );
    } catch (issue) {
  ...
  function exportPrintSheet() {
    try {
      downloadCardFile(
        ...
        `${cardExportFilename(project.name)}-cards-${paper}.html`,
  ...
  return (
    <section className="card-studio" aria-label="Card Studio">
      ...
      <div className="card-studio-workspace" data-region="card-studio-workspace">
        <section className="card-studio-data-panel" aria-label="Card designs">
          <header className="card-studio-table-toolbar">
            ...
            <div className="card-studio-toolbar" data-region="card-table-actions">
              ...
              <button type="button" className="card-studio-button" onClick={() => setAddingField(!addingField)}>
                <Plus size={14} /> Field
              </button>
          ...
          {view === "table" ? (
            <CardTable
              ...
              onChange={updateRow}
              onDuplicate={addRow}
              onRemove={(id) => update({ ...state, rows: state.rows.filter((row) => row.id !== id) })}
              onAdd={() => addRow()}
              onSamples={() => {
            ...
            <div className="card-studio-deck-scroll" data-region="card-deck-preview">
              {expanded.length ? (
                ...
                <div className="card-studio-empty" data-region="card-deck-empty">
                  <Layers3 size={40} />
                  <h3>{errors.length ? "A few details need your attention." : "Your deck is waiting to happen."}</h3>
                  <p>
                    {errors.length
          ...
          <footer className="card-studio-table-footer">
            <span>
              {view === "table"
                ? "Each row is a design. Copies turns it into a deck. Set copies to 0 to keep a draft."
                : `${expanded.length ? currentPage * 48 + 1 : 0}–${Math.min((currentPage + 1) * 48, expanded.length)} of ${expanded.length} cards · live template preview`}
            </span>
      ...
      <footer className="card-studio-footer">
        <div className="card-studio-batch-state" data-region="card-batch-state">
          ...
          <span>
            <strong>
              {batchIsCurrent
                ? `${state.generated?.cards.length ?? total} cards generated`
                : state.generated
                  ? "Your deck has new changes"
            ...
            <small>
              {batchIsCurrent
                ...
                : state.generated
                  ...
                  : "Batch your cards, then take them to the table."}
            </small>
          </span>
        </div>
        <div className="card-studio-export-actions" data-region="card-export-actions">
          <button
            ...
            onClick={() => {
              downloadCardFile(
                ...
                `${cardExportFilename(project.name)}-cards.csv`,
                "text/csv;charset=utf-8",
              );
              setStatus("Card table downloaded as CSV, including your custom fields.");
            }}
            title="Download the editable card table"
          >
          ...
          <button
            ...
            title="Download the selected card as an SVG"
          >
            SVG
          </button>
          ...
          <button
            ...
            onClick={generate}
          >
            <Layers3 size={15} /> Generate {total || ""} cards
          </button>
        </div>
      </footer>
      {importing && (
        <ImportCardsDialog
          ...
          onImport={(result, replace) => {
            ...
            setView("table");
            setSelectedId(rows[0]?.id);
            setStatus(`Imported ${rows.length} card designs. Extra columns are ready to use as template fields.`);
          }}
        />
      )}
    </section>

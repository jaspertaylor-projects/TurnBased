...
export function normalizeShape(text: string): string | null {
  ...
  const sortedKeys = Object.keys(SHAPE_SYNONYMS).sort(
...
export function detectCategory(slug: string): {
  category: string;
  subcategory: string;
  shape: string | null;
} {
  ...
  // Boards
  if (s.includes('game-board'))
    return {
      category: 'boards',
      subcategory: deriveBoardSubcategory(s),
      shape,
    };
...
export function defaultLayout(category: string) {
  switch (category) {
    ...
    case 'tiles':
      return { widthMm: 50, heightMm: 50, bleedMm: 1, safeZoneMm: 1, dpi: 300 };
    default:
      return { widthMm: 63, heightMm: 88, bleedMm: 3, safeZoneMm: 2, dpi: 300 };
  }
}

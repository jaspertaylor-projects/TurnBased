...
For the product overview and quickstart, see the [root README](../README.md).
This guide covers the local stack, configuration, browser tools, and recovery
from common development problems. See [verification](verification.md) for checks.
...
Set `RESET_DB=1` to rebuild local Supabase from migrations and seed data.
This removes local Supabase data; the separate catalog database is preserved:
...
- `OPENROUTER_API_KEY` — required for any AI-backed function
  (`ai-project-builder`, `ai-rules-writer`, `ai-card-table`, `ai-image-agent`).
- `OPENROUTER_DEFAULT_MODEL` — fallback text-model ID for project building,
  rules writing, and card-table AI when no per-function override is set.
- `OPENROUTER_BUILD_MODEL` — override for `ai-project-builder` only.
- `OPENROUTER_RULES_MODEL` — rules-writer model override; also a fallback
  for card-table AI. Supported models are validated against the model catalog.
- `OPENROUTER_CARD_TABLE_MODEL` — model override for `ai-card-table`. See its
  [API contract](../supabase/functions/ai-card-table/README.md) for model
  precedence, validation, limits, and usage accounting.
...
from `supabase/functions/`, or the CLI serve worker has not registered a
new function. Restart `supabase functions serve` as described in Common
operations; restarting the Docker container alone retains its old route list.
